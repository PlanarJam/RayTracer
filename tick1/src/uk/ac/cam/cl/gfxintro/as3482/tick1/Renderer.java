...
public class Renderer {
	...
	protected ColorRGB trace(Scene scene, Ray ray, int bouncesLeft) {
		...
		// Recursive case
			...
			Vector3 direction = ray.getDirection().scale(-1).reflectIn(N).normalised();

			// Spawn a reflectedRay with bias
			Ray reflectedRay = new Ray(P.add(N.scale(EPSILON)), direction);
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			// Determine if shadowRay intersects with an object before it reaches the pointlight
			if (scene.findClosestIntersection(shadowRay).getDistance() > distanceToLight) {

				// If it does not, add diffuse/specular components
				colourToReturn = colourToReturn.add(C_diff.scale(k_d).scale(I).scale(Math.max(0, N.dot(L)))).add(C_spec.scale(k_s).scale(I).scale(Math.pow(Math.max(0, R.dot(V)), alpha)));
			}
		}
